...
def t_period(df,plt_type):
    ...
    class MyGraph(FigureCanvas):
        ...
        def __init__(self, parent=None, width=5, height=4, dpi=100):
            ...
            plt.figure(figsize=(10,5))
            if (plt_type == 'semilog'):
                self.axes.semilogx(df['t'][1:], df['P_dash'][1:],linestyle='--', marker='o', color='b',label='Derivative Plot')
                self.axes.semilogx(df['t'][1:], df['delP'][1:],linestyle='--', marker='o', color='r',label="delP and (delP)', psia")
                self.axes.set_title("Semi-Log Plot")
            elif (plt_type == "log"):
                self.axes.loglog(df['t'][1:], df['P_dash'][1:],linestyle='--', marker='o', color='b',label='Derivative Plot')
                self.axes.loglog(df['t'][1:], df['delP'][1:],linestyle='--', marker='o', color='r',label="delP and (delP)', psia")
                self.axes.set_title("Log-Log Plot")

            self.axes.set_xlabel("Time, hrs")
            self.axes.set_ylabel("delP and P' (t*dp/dt), psia")
